...
LoginSystem::LoginSystem() {
    loadData();
    createAdmin();//создание админа по умолчанию 
}
...
shared_ptr<EmployeeUser> LoginSystem::signUp() {
    ...
    // АВТОМАТИЧЕСКАЯ РОЛЬ "user" ДЛЯ ВСЕХ НОВЫХ ПОЛЬЗОВАТЕЛЕЙ
    string role = "user";
    ...
    loginToSalt[login] = salt;
    loginToHashedPassword[login] = hash;
    loginToRole[login] = role; // Все новые пользователи получают роль "user"
...
Base* LoginSystem::run()
{
    ...
    do {
        ...
        case 2:
            ...
            break;
        case 3:
            return new GuestEmployeeBase(); // Сразу возвращаем гостя
        case 0:
            return nullptr;
        }
    } while (!user);
    ...
    while (true) {
        ...
        switch (choice) {
        case 1:
            // ПРАВИЛЬНОЕ СООТВЕТСТВИЕ РОЛЕЙ И ИНТЕРФЕЙСОВ
            if (role == "admin") {
                //std::cout << "Создается AdminEmployeeBase\n";
                return new AdminEmployeeBase(user);
            }
            if (role == "user") {
               // std::cout << "Создается UserPayrollBase\n";
                return new UserPayrollBase(user);
            }
            // Если роль какая-то другая - гость
           // std::cout << "Создается GuestEmployeeBase\n";
