...
/* void directoryChecker(int code) {
    if (code == EEXIST || code == -1) {
        std::clog << "Каталог уже существует.\n";
        return;
    }
    if (code == ENOENT) {
        std::clog << "Путь не найден.\n";
        throw std::exception("FileSystem error");
    }
    std::clog << "Каталог создан.\n";
    bool checkInpFile(std::ifstream& f) {
    if (!f) {
        std::clog << "Ошибка открытия файла.\n";
        return false;
    }
    f.seekg(0, std::ios::end);
    if (f.tellg() == 0) {
